...
function ResultContent() {
  ...
  if (!mbtiInfo) {
        return <div>잘못된 MBTI 유형입니다.</div>div>;
  }
          ...
          <main className="min-h-screen flex flex-col items-center p-4 py-8">
            {/* Result Card */}
                <motion.div
                        ...
                        <p className="text-gray-400 mb-2">당신의 MBTI는</p>p>
                        ...
                        <p className="text-gray-500 leading-relaxed">{mbtiInfo.description}</p>p>
                </motion.div>motion.div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {keywords.map((kw) => (
                                      <motion.button
                                                    ...
                                                    >
                                                    <span className="text-lg">{kw.emoji}</span>span>
                                                    <br />
                                        {kw.label}
                      ...
                      {loading ? (
                                            <div className="flex items-center justify-center py-8">
                                                          <motion.div
                                                                          ...
                                                                          />
                                                          <span className="ml-3 text-gray-400">AI가 분석 중이에요...</span>span>
                                            </div>div>
                                          ) : (
...
export default function ResultPage() {
    return (
          <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><p>로딩 중...</p>p></div>div>}>
                <ResultContent />
          </Suspense>Suspense>
        );
}</div>
